%% FOR TEST ONLY
function [U, S, V, max_rnk, matrix_norm_sq] = mp_compress_m(A, method, vareps, varargin)
    
    if nargin == 4
        max_rnk = varargin{1};
        norm_type = '2';
        issparse = true;
    elseif nargin == 5
        max_rnk = varargin{1};
        norm_type = varargin{2};
        issparse = true;
    elseif nargin == 6
        max_rnk = varargin{1};
        norm_type = varargin{2};
        issparse = varargin{3};
    else 
        max_rnk = 999;
        norm_type = 'fro';
        issparse = false;
    end 

    [m, n] = size(A);
    min_size = min([m, n]);

    if max_rnk > min_size
        max_rnk = min_size;
    end

    if min_size == 0
        U = zeros(m, 0);
        V = zeros(0, n);
        return;
    end

    if strcmp(method, 'svd')
        [U, S, V] = svd(full(A), 'econ');
        if norm_type == '2'
            rnk = sum(abs(diag(S)) > S(1,1) * vareps);
            if rnk > max_rnk
                rnk = max_rnk;
            end
            U = U(:,1:rnk);
            S = diag(S(1:rnk,1:rnk));   % diagonal format
            V = V(:,1:rnk);
            matrix_norm_sq = sum(S.^2);    % (Frobenius norm)^2
        elseif norm_type == 'fro'
            sq_dS = diag(S).^2;
            normf = sum(sq_dS);
            cusm = cumsum(sq_dS, "reverse") / normf;
            in_eq = cusm > vareps^2;
            rnk = min(sum(in_eq) + 1, size(U, 2));
            if rnk > max_rnk
                rnk = max_rnk;
            end
            U = U(:,1:rnk);
            S = diag(S(1:rnk,1:rnk));   % diagonal format
            V = V(:,1:rnk);
            matrix_norm_sq = sum(S.^2);    % (Frobenius norm)^2
        else
            error("This norm type is not sopported for truncation. Please use `fro` or `2` for norm type specification.")
        end 

    elseif strcmp(method, 'rsvd')
        [U, S, V] = rsvd(A, max_rnk, 1);
        if norm_type == '2'
            rnk = sum(abs(diag(S)) > S(1,1) * vareps);
            U = U(:,1:rnk);
            S = diag(S(1:rnk,1:rnk));   % diagonal format
            V = V(:,1:rnk);
            matrix_norm_sq = sum(S.^2);    % (Frobenius norm)^2
        elseif norm_type == 'fro'
            sq_dS = diag(S).^2;
            normf = sum(sq_dS);
            cusm = cumsum(sq_dS, "reverse") / normf;
            in_eq = cusm > vareps^2;
            rnk = min(sum(in_eq) + 1, size(U, 2));
            if rnk > max_rnk
                rnk = max_rnk;
            end
            U = U(:,1:rnk);
            S = diag(S(1:rnk,1:rnk));   % diagonal format
            V = V(:,1:rnk);
            matrix_norm_sq = sum(S.^2);    % (Frobenius norm)^2
        else
            error("This norm type is not sopported for truncation. Please use `fro` or `2` for norm type specification.")
        end 

    elseif strcmp(method, 'qr')  %%% Not correct chop is diabled
        [U, V, P] = qr(A);
        % V = mchop(V);
        rnk = sum(abs(diag(V)) > V(1,1) * vareps);
        % U = mchop(U(:, 1:rnk));
        % V = mchop(V(1:rnk,:)*P');
    end
    
    if issparse
        U = sparse(U);
        V = sparse(V);
    end

    max_rnk = rnk;
end


function [U, S, V] = rsvd(A, k, p)
    n = size(A, 2);
    Omega = randn(n, k+p);
    [Q,~,~] = qr(A*Omega, "econ");
    [U,S,V]  = svd(Q'*A);
    U = Q*U(:,1:k);
    S = S(1:k,1:k);
    V = V(:,1:k);
end